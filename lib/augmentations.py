...
class Compose(object):
    ...
    def __call__(self, img, depth, imobj=None):
        for t in self.transforms:
            img, depth, imobj = t(img, depth, imobj)
        return img, depth, imobj
...
class ConvertToFloat(object):
    ...
    """
    def __call__(self, image, depth, imobj=None):
        return image.astype(np.float32), depth.astype(np.float32), imobj
...
class RandomMirror(object):
    ...
    def __call__(self, image, depth, imobj):
...
class Augmentation(object):
    ...
    def __init__(self, conf):
        ...
        self.depth_mean = conf.depth_mean
        self.depth_std = conf.depth_std
        ...
        if conf.distort_prob <= 0:
            self.augment = Compose([
                ...
                Resize(self.size),
                Normalize(self.mean, self.stds, self.depth_mean, self.depth_std, self.use_rcnn_pretrain, self.use_sne)
            ])
        else:
            self.augment = Compose([
                ConvertToFloat(),
                ...
                RandomMirror(self.mirror_prob),
                Resize(self.size),
                Normalize(self.mean, self.stds, self.depth_mean, self.depth_std, self.use_rcnn_pretrain)
            ])

    def __call__(self, img, depth, imobj):
        return self.augment(img, depth, imobj)
...
class Preprocess(object):
    ...
    """
    def __init__(self, size, mean, stds, depth_mean, depth_std, use_rcnn_pretrain, use_sne):
