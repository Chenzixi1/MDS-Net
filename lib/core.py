...
from lib.lr import OneCycleLR

from scripts.config.yolof_config import Config
...
def adjust_lr(conf, optimizer, iter, scheduler):
    ...
    if conf.solver_type.lower() == 'sgd':
        ...
        # def get_lr(self):  # Sin
        #     return [
        #         self.eta_min + (base_lr - self.eta_min) * (1 + math.cos(math.pi * self.T_cur / self.T_i + math.pi)) / 2
...
def next_iteration(loader, iterator):
    ...
    # next batch
    try:
        images, depths, imobjs = next(iterator)
    ...
    return iterator, images, depths, imobjs
